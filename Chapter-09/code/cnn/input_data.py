...
data1 = np.load('../../data/label.npz')

sst = data1['sst'][:].reshape(-1, 1, 6, 27)
mld = data1['mld'][:].reshape(-1, 1, 6, 27)

data2 = np.load('../../data/solar_radiation.npz')
print(data2.files)
dlwrf1 = data2['dlwrf1'][:]
dswrf1 = data2['dswrf1'][:]
lhtfl1 = data2['lhtfl1'][:]
...
data3 = np.load('../../data/deep_variables.npz')
T_d1 = data3['T_d1'][:]
v_d1 = data3['v_d1'][:]
u_d1 = data3['u_d1'][:]

data4 = np.load('../../data/other_variables.npz')
mld1 = data4['mld1'][:]
sst_feature = data4['sst_feature'][:]
vflx1 = data4['vflx1'][:]
uflx1 = data4['uflx1'][:]

data5 = np.load('../../data/coordinate_variables.npz')
xx1 = data5['xx1'][:]
yy1 = data5['yy1'][:]
...
train_data = torch.cat((sst1_train, xx_train, yy_train, Qnet_train, mld_train, u_train, v_train, T_d_train, v_d_train),
                       dim=1)

valid_data = torch.cat((sst1_valid, xx_valid, yy_valid, Qnet_valid, mld_valid, u_valid, v_valid, T_d_valid, v_d_valid),
                       dim=1)
