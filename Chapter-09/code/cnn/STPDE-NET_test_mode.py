...
model = ConvNet(input_dim=3,hidden_dim=1,kernel_size1=(3,3,3),padding1=(1,1,1),kernel_size2=(1,3,3),padding2=(0,1,1))
...
s = rmse(test_label, pred_test1.detach().cpu().numpy())
print('RMSE: {:.3f}'.format(s))

test_label = np.array(test_label)
pred_test1 = np.array(pred_test1.detach().cpu().numpy())
...
Qnet_1 = np.array(Qnet.detach().cpu().numpy())
ZAdv_1 = np.array(ZAdv.detach().cpu().numpy())
MAdv_1 = np.array(MAdv.detach().cpu().numpy())
VAdv_1 = np.array(VAdv.detach().cpu().numpy())
Tend1 = np.array(Tend.detach().cpu().numpy())

Tend_true_1 = np.array(Tend_true.detach().cpu().numpy())
