...
class DSBRunner:
    def __init__(self, opt, log):
        ...
        self.mim_ssim = -float('inf')
        self.mim_psnr = -float('inf')
        self.mim_rmse_epoch = 0
        self.mim_mse_epoch = 0
        self.mim_mae_epoch = 0
        self.mim_r2_epoch = 0
        self.mim_psnr_epoch = 0
        self.mim_ssim_epoch = 0
        self.mim_rmse_batch = 0
        self.mim_mse_batch = 0
        self.mim_mae_batch = 0
        self.mim_r2_batch = 0
        self.mim_psnr_batch = 0
        self.mim_ssim_batch = 0
        ...
        self.max_mean_psnr = -float('inf')
        self.max_mean_ssim = -float('inf')

        self.min_mean_rmse_epoch = 0
        self.min_mean_mse_epoch = 0
        self.min_mean_mae_epoch = 0
        self.max_mean_r2_epoch = 0
        self.max_mean_psnr_epoch = 0
        self.max_mean_ssim_epoch = 0
         # 异步调度参数
    ...
    def validate(self, epoch, val_loader, opt):
        self.net.eval()
        total_mse, total_rmse, total_mae, total_r2 , total_psnr, total_ssim= [], [], [], [], [], []
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y) in enumerate(tqdm(val_loader)):
                ...
                SSIM = pytorch_ssim.ssim(reconstructed_image, x0)
                total_ssim.append(SSIM.item())

                PSNR = 20 * torch.log10(255 / torch.sqrt(mse_score))
            ...
            if epoch >= opt.visual_epoch:
                ...
                if self.max_mean_psnr <= np.mean(total_psnr):
                    self.max_mean_psnr = np.mean(total_psnr)
                    self.max_mean_psnr_epoch = epoch
                if self.max_mean_ssim <= np.mean(total_ssim):
                    self.max_mean_ssim = np.mean(total_ssim)
                    self.max_mean_ssim_epoch = epoch
        ...
        result = [self.min_mean_rmse, self.min_mean_rmse_epoch, self.min_mean_mse, self.min_mean_mse_epoch,
                 ...
                 self.max_mean_ssim, self.max_mean_ssim_epoch, self.max_mean_psnr, self.max_mean_psnr_epoch]
        with open("result_{}_{}".format(opt.N_S_ratio, opt.corrup_rate), 'w') as file:
            file.write("best: Rmse:" + str(result[0]) + "[Epoch:" + str(result[1]) + "] "
                       + "r2:" + str(result[6]) + "[Epoch:" + str(result[7]) + "]"
                       + "ssim:" + str(result[8]) + "[Epoch:" + str(result[9]) + "]"
                       + "psnr:" + str(result[10]) + "[Epoch:" + str(result[11]) + "]"
        ...
        print("batch最高psnr:{:.8f}  [Epoch{}_batch{}]".format(self.mim_psnr, self.mim_psnr_epoch, self.mim_psnr_batch))
        print("batch最高ssim:{:.8f}  [Epoch{}_batch{}]".format(self.mim_ssim, self.mim_ssim_epoch, self.mim_ssim_batch))
        print('Min_mean_rmse:{:.8f} [Epoch{}]\t Min_mean_mse:{:.8f} [Epoch{}]\t Min_mean_mae:{:.8f} [Epoch{}]\t Max_mean_r2:{:.8f} [Epoch{}]\t Max_mean_psnr:{:.8f} [Epoch{}]\t Max_mean_ssim:{:.8f} [Epoch{}]\t'
              .format(self.min_mean_rmse, self.min_mean_rmse_epoch, self.min_mean_mse, self.min_mean_mse_epoch,
                     self.min_mean_mae, self.min_mean_mae_epoch, self.max_mean_r2, self.max_mean_r2_epoch,self.max_mean_psnr, self.max_mean_psnr_epoch,self.max_mean_ssim, self.max_mean_ssim_epoch))
