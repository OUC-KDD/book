...
### I2SB-SST 训练与重建（run.py + diffusion.py + model.py）

使用 Schrödinger Bridge 扩散过程，将损坏图 $x_1$ 映射回完整图 $x_0$；在云遮挡区域内实现 SST 重建，并输出 RMSE / MAE / R² / SSIM / PSNR 等指标。

## 📊 一、数据处理（`Visual_Tool.py`）

### 1. 数据来源与区域设置

本项目直接使用 预处理完成的 SST H5 数据集（与 DINCAE 实验保持一致）。数据来源与设置如下：
数据源：国家卫星海洋应用服务（NSOAS）SST 四级产品（Multi-mission L4）
时间范围：2022 年 1 月 – 2023 年 4 月
空间范围：大西洋海域
纬度：23°N – 26°N
经度：70° W– 67°W
空间分辨率：约 5 km × 5 km
完整性：L4 SST 本身无缺失，作为 $x_0$（完整真值）；在此基础上叠加真实云掩模与噪声构造 $x_1$（损坏输入）。
为构建不同缺失程度与噪声水平的训练数据，本数据集集成：
真实云掩模：来自 WHU Cloud Dataset（用于模拟云遮挡缺失）
高斯噪声：标准正态分布噪声，用于模拟不同 N/S 比（0.1、0.2、0.3）

### 2. 数据存放位置与命名规则

所有训练/验证用的 .h5 文件已统一存放在：New_data/FNO_based_2024_2/data/
文件命名格式为：{N_S_ratio}_{mask_type}_{corrup_rate}_train_South_Sea_miss.h5
其中：

```
mask_type = Cloud_mask：云遮挡类型（真实云掩模）
corrup_rate ∈ {8, 25, 46, 68}：云覆盖率设置
N_S_ratio ∈ {0.1, 0.2, 0.3, ...}：噪声/信号比设置
miss / true：分别表示损坏输入 $x_1$ 与完整真值 $x_0$
```

说明：训练脚本通过 deal_sst_util.read_cache() 直接读取上述 .h5 文件，并构建 TensorDataset 用于 I2SB 模型训练与验证。
...
对应理论上：
"
𝑞(𝑥𝑡∣𝑥0,𝑥1)=𝑁(𝑥𝑡∣𝜇0𝑥0+𝜇1𝑥1,𝜎SB2)
"
...
实现：从损坏图 x1 沿 Schrödinger Bridge 逐步去噪，同时保持非云区强约束；
...
直接执行：python run.py
程序会：读取对应 .h5 数据；进行 I2SB 训练与验证；自动输出重建可视化图像与定量评估结果。

## 🧱 六、环境依赖

建议环境：Python ≥ 3.8；PyTorch ≥ 1.10
依赖建议：
